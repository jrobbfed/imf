...
int readCatalogue(pulsar *psr,linkedList *list,int *npsr,char name[MAX_PSR][MAX_NAMELEN],
		  ...
		  char alias1[MAX_ALIASES][MAX_STRLEN],char alias2[MAX_ALIASES][MAX_STRLEN],int aliasCount)
{
  FILE *fin;
      ...
      if (line[0]=='@')
	{
	  ...
	  if (merge!=1 && filter==0 && strlen(boundary)>0) /* Have boundary for position */
 	  ...
 	  if (willmerge==0 && strlen(filterStr)>9 && filter==0) /* logical filter expression */
	    {
	      if (formExpression(filterStr,&psr[*npsr],&badval)==1)
		filter=0;
	      else
		filter=1;
	    }
...
double formExpression(char *filterStr,pulsar *psr,int *badval)
{
  int ip,i,errorFlag;
  ...
  if (nread != 3)
      ...
      else
	{
	  if (strcmp(label,"S1400")!=0 && strcmp(label,"S400")!=0 && 
	      ...
	      && strcmp(label,"S1600")!=0  && strcmp(label,"W10")!=0
	      && strcmp(label,"W50")!=0)
