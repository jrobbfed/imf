		   ...
		   int nohead,int tbForm,int noNumber,int webflag)
{
  ...
  if (nohead==0)
      ...
      for (j=0;j<np;j++)
	{
	  ...
	  else if (tbForm==2 || tbForm==3) /* short       */
	    {
	  ...
	  /* Room for error */
	  if (pcat_errZero[ip]!=0 && tbForm==0)
	    {
	      strcat(headerStr,"      ");
	      strcat(headerStr2,"      ");
	    }
	  /* Room for reference */
	  if (pcat_refZero[ip]!=0 && tbForm==0)
	    {
	      strcat(headerStr,"            ");
	      strcat(headerStr2,"            ");
	    }
  ...
  for (i=0;i<npsr;i++)
      ...
      for (j=0;j<np;j++)
	{
	  ...
	  if (webflag==1 && ptr[j] == 2 && dispParam.set1==1 && (tbForm==0 || tbForm==1))
	    {
	      plusSign=0;
	  ...
	  else if (webflag==1 && dispParam.set1==1 && (tbForm==0 || tbForm==1))
	    {
	  ...
	  else if (dispParam.set1==1 && (tbForm==2 || tbForm==3)) 
	    {
	  ...
	  else if (tbForm==2 || tbForm==3) /* NULL string for SHORT */
	    {
	      strcpy(temp,defParam[ptr[j]].shortStr);
	      strptr = strtok(temp,"%.-sgef");
	  ...
	  else if ((tbForm==3 || tbForm==4) && pcat_errZero[ptr[j]]!=0)
	    {
	      if (dispParam.set2==1)
		{
	  ...
	  else if (tbForm!=2 && tbForm!=3 && tbForm!=1 && tbForm!=4 && tbForm!=5 && pcat_refZero[ptr[j]]!=0)printf(" %-10s ",NO_REF);
	}
	...
	{
	  puts("");
	  if ((i+1)%5==0 && nohead==0 && noNumber==0 && i!=npsr-1)puts("");
	}
