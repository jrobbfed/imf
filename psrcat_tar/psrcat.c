...
int main(int argc,char *argv[])
{
  ...
  if (readCatalogue(psr,list,&npsr,selectNames,iname,haveName,filterStr,ptr,sptr,np,dbFile,0,strlen(mergeCat),boundary,defParam,checkCat,alias1,alias2,aliasCount)==1)
	  ...
	  while (!feof(groupFile))
	    {
	      ...
	      if (nread==1) readCatalogue(psr,list,&npsr,selectNames,iname,haveName,filterStr,ptr,sptr,np,tempMerge,1,1,boundary,defParam,checkCat,alias1,alias2,aliasCount);      	  
	    }
	  pclose(groupFile);
  ...
  if (strlen(mergeCat)>0)
	...
	temp[i] = list[i];
      ...
      for (i=0;i<npsr;i++)
	{
	  if (strlen(filterStr)<10 || formExpression(filterStr,check,&badval)==1) /* Don't filter */
	    {
	      ...
	      if (ephem==2 || ephem==4 || (defParam[j].ephem==1 && ((dispParam.derived!=1 || j==if0 || j==if1) && j!=ip0 && j!=ip1)))
		{
		  ...
		  if (strcmp(pcat_parameters[j],"C1")!=0 &&
				      ...
				      || psrptr->param[ip5].set2==1))
			{
		      ...
		      if (dispParam.set1==1) {
			printf("%-15.15s %-25.25s",pcat_parameters[j],dispParam.val);
			if (dispParam.set2==1)
			  {			    
	  ...
	  for (j=0;j<np;j++)
	    {
	      ...
	      if (dispParam.set1==1) {
		printf("%-15.15s %-25.25s",pcat_parameters[ptr[j]],dispParam.val);
		if (dispParam.set2==1)
		  {
		    sscanf(dispParam.err,"%d",&ierr);
