...
void defineCustom(pulsar *psr,int ip,int type);
...
void deriveParameter(char *str,pulsar *psr,int ip)
{
  ...
  /* Custom parameters */
  /* ***************** */
  if (strcmp(str,"C1")==0) defineCustom(psr,ip,1);
  else if (strcmp(str,"C2")==0) defineCustom(psr,ip,2);
  else if (strcmp(str,"C3")==0) defineCustom(psr,ip,3);
  else if (strcmp(str,"C4")==0) defineCustom(psr,ip,4);
}
...
void defineCustom(pulsar *psr,int ip,int type)
{
  ...
  if (type==1) psr->param[ip].shortVal = v1 = formExpression(pcat_custom1,psr,&badval);
  if (type==2) psr->param[ip].shortVal = v1 = formExpression(pcat_custom2,psr,&badval);
  if (type==3) psr->param[ip].shortVal = v1 = formExpression(pcat_custom3,psr,&badval);
  if (type==4) psr->param[ip].shortVal = v1 = formExpression(pcat_custom4,psr,&badval);
  if (badval==0)
    {
...
void defineParameters(paramtype *defParam)
{
  ...
  addParameter(defParam,"ASSOC","%-35.35s","ASSOC"," ","Association with SNR, GC etc.",5,0);
  ...
  addParameter(defParam,"PBDOT","%10.5f","PBDOT"," ","1st time-deriv of binary period",3,1);
  ...
  addParameter(defParam,"ECCDOT","%10.5f","ECCDOT"," ","Rate of change of eccentricity",3,1);
  addParameter(defParam,"A1DOT","%10.5f","A1DOT"," "," ",3,1);
  ...
  addParameter(defParam,"A12DOT","%10.5f","A12DOT","(lt-s s^-2)","Second time derivative of projected semi-major axis of orbit (lt-s s^-2)",3,1);
